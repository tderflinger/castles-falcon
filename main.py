...
def setup_wikidata_mapping(engine, table_name='string'):
...
def ensure_output_schema(engine):
...
def parse_args():
    parser = argparse.ArgumentParser(description='Extend castles data with Wikidata attributes.')
    parser.add_argument(
        'wikidata_db',
...
def main(wikidata_db):
    osm_property_ids = [1000010689, 1000000402]
    website_property_id = 1000000856
    ...
    with Session(wikidata_engine) as wikidata_session:
        for i, castle in enumerate(rows, 1):
            ...
            first_match = (
                ...
                .filter(WikiData.property_id.in_(osm_property_ids + [website_property_id]))
                .all()
            )
            if not first_match:
                continue
            ...
            osm_id_value = next(
                (
                    ...
                    if item.property_id in osm_property_ids and getattr(item, 'string', None)
                ),
                None,
            )
            #if osm_id_value is None:
            #    continue
            ...
            website_value = next(
                (
                    getattr(item, 'string', None)
                    for item in first_match
                    if item.property_id == website_property_id and getattr(item, 'string', None)
                ),
                None,
            )
